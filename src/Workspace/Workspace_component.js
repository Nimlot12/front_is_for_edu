...
const Workspace_component = () => {
    ...
    useEffect(() => {
        // Получаем токен из localStorage (предполагается, что он там сохраняется после авторизации)
        const token = localStorage.getItem("access_token");

        fetch("http://localhost:8000/workspaces/get_by_workspaces_list_from_user/", {
        headers: {
        ...
        .then((response) => {
            if (!response.ok) {
            throw new Error("Ошибка при загрузке рабочих областей");
            }
    ...
    if (loading) {
        return <p>Загрузка рабочих областей...</p>;
    }

    return(
        <div className="workspaces-grid" id="workspacesList">
            {workspaces.map((workspace) => (
                <NavLink to={`/lead_page/teach_panel/workspaces/lesson_homework/${workspace.id}`} style={{
                                                                                            all: 'unset',       
                                                                                            display: 'block',  
                                                                                        }}>
